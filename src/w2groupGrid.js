 ...
 *        - w2groupGrid        - grid widget
 *        - $().w2groupGrid    - jQuery wrapper
 ...
 *
 * == TODO ==
 * local sort,search - not working
 * local regroup  - not working
 * local data - optimize record grouping
 * server search - not all columns are sent to server on search all fields
 *
 ************************************************************************/

(function() {
    var w2groupGrid = function(options) {
    ...
    $.fn.w2groupGrid = function(method) {
        if (typeof method === 'object' || !method) {
            // check name parameter
            if (!w2utils.checkName(method, 'w2groupGrid')) return;
            // remember items
            ...
            // extend items
            /**
             * starting new implementation here
             * @type {w2groupGrid}
             *
             */ // we are at first level, so we take first column as groupColumn



            var object = new w2groupGrid(method);
            $.extend(object, {
                postData: {},
                records: [],
                columns: [],
            ...
            object.defaultOptions = method;

            object.parentGrid = method.parentGrid || method.name;
            // reassign variables
            ...
            /**
             * Extend original post data with grouping options
             * @type {[type]}
             */

            object.postData = method.postData || {};
            object.postData.group_data = object.postData.group_data || [];

            if (method.groupData && method.groupData.length > 0) {
                if (!method.groupColumn) {

                    object.groupColumn = method.groupData[0].field;

                    object.postData.group_data.push({
                        groupColumn: object.groupColumn
                    });
                    object.postData.groupOn = object.groupColumn;
                    method.parentGrid = method.name; // this is parent grid
                    object.expandedGrids = [];
                    object.defaultRecords = method.records;
                    object.defaultColumns = columns;
                }
            }
            // setting additional postData
            var pd = $.extend(method.postData, true);

            // /**
            //  * we are in a child grid
            //  * 
            //  * @param  {[type]} e) {                           
            //  * return e.parentGroup [was set in showGroup function on toggle group]
            //  * parent group is the column group of parent grid 
            //  * @return {[type]}    [description]
            //  * 
            //  */

            // var result = $.grep(pd.group_data, function(e) {
            //     return e.parentGroup === method.parentGroup;
            // });

            // if (result.length === 0 && !method.isLastGroup) { // if item not exists, we create one taking data that comes from expanded grid
            //     var o = {
            //         groupColumn: method.groupColumn,
            //         parentGroup: method.parentGroup,
            //         parentGroupValue: method.parentGroupValue
            //     };
            //     pd.group_data.push(o);          
            // } else if (result.length > 0) {
            //     //we have to modify the group_data if an item already exists
            //     result[0].parentGroupValue = method.parentGroupValue;
            //     result[0].parentGroup = method.parentGroup;
            //     result[0].groupColumn = method.groupColumn;
            // }
            // pd.groupOn = method.groupColumn;

            if (method.summaryData) {
                pd.summaryData = method.summaryData;
            }

            //object.postData = $.extend(postData, pd);
            object.isLastGroup = method.isLastGroup || object.groupData.length === 0;
            object.show.expandColumn = !object.isLastGroup;
            // check if there are records without recid
            if (!method.url) {
                records = object.getGroupedRecords();
            }
            for (var r in records) {

                if (records[r].recid === null || typeof records[r].recid === 'undefined') {
                    if (method.recid) {
                        records[r].recid = records[r][method.recid];
                    } else {
            ...
            console.log('ERROR: Method ' + method + ' does not exist on jQuery.w2groupGrid');
        }
    }
    ...
    w2groupGrid.prototype = {
        ...
        initGroup: function(method, box, column, value) {
            if (typeof method === 'object' || !method) {
                // check name parameter
                if (!w2utils.checkName(method, 'w2groupGrid')) return;
                // remember items
                ...
                // check if there are records without recid
                if (!method.url) {
                    //records = object.getRecordsOfGroup(column, value);
                    var rs = _.find(method.records, function(rec) {
                        return rec[column] === value;
                    });
                    records = this.constructRecords(rs.childRecords, method.groupColumn);
                }
                ...
                // register new object
                w2ui[object.parentGrid].childrenGrids[object.name] = object;
                return object;
        ...
        showGroup: function(recid) {
            var obj = this;
            //get parent group
            var parentGroup = this.groupColumn;
            var name = this.name + '_grp_' + recid;
            var box_id = 'grid_' + obj.name + '_rec_' + recid + '_expanded';
            var parent = obj.name.indexOf('_grp_') !== -1 ? name.substring(0, name.indexOf('_grp_') + 6) : name.substring(0, name.indexOf('_grp_' + recid));
            var mainGrid = w2ui[obj.parentGrid];
            var isexpanded = $.grep(mainGrid.expandedGrids, function(e) {
                return e.grid === name;
            });
            if (isexpanded.length === 0) {
                mainGrid.expandedGrids.push({
                    parent: parent,
                    grid: name,
                    parentGrid: obj.name,
                    id: recid
            ...
            // if (w2ui.hasOwnProperty(obj.name + '_grp_' + recid)) { // check if grid exist
            //     w2ui[obj.name + '_grp_' + recid].destroy();
            // }
            var record = obj.get(recid); //get the expanded record

            var tmpOptions = $.extend({}, {}, this.defaultOptions); //extend options
            var column, value;
            // pass sort and search data from parent grid
            //var sortData = options.sortData;
            tmpOptions.sortData = mainGrid.sortData;

            tmpOptions.searchData = mainGrid.searchData;

            tmpOptions.name = obj.name + '_grp_' + recid;
            tmpOptions.show = $.extend({}, obj.show, {});
            tmpOptions.show.columnHeaders = false;
            tmpOptions.toolbar = undefined;
            tmpOptions.show.toolbar = false;
            tmpOptions.fixedBody = true;
            tmpOptions.parentGrid = mainGrid.name;

            // tmpOptions.url = obj.url;
            if (!tmpOptions.url) {
                tmpOptions.records = obj.records;
                column = obj.groupColumn;
                value = record[parentGroup];
            }

            //resize first column
            //var col = w2ui[options.name].getColumn(tmpOptions.columns[0].field);
            //var sz = col.sizeCalculated.replace('px', '');
            //tmpOptions.columns[0].size = (sz - 16) + 'px';


            //get the index of parent group
            var groupColumn;
            $.map(obj.groupData, function(ob, index) {
                if (ob.field == parentGroup) {
                    groupColumn = obj.groupData[index + 1] ? obj.groupData[index + 1].field : undefined;
                }
            });
            // get the column to group on - we search the groupData array and take the next group object



            var parentGroupValue = record[parentGroup]; //get the value that the curend expanded grid will group on



            if (groupColumn) { // we have a column to group on - we will render a grouped grid
                tmpOptions.groupColumn = groupColumn;
                // tmpOptions.parentGroup = parentGroup;
                // tmpOptions.parentGroupValue = parentGroupValue;

                var pd = $.extend({}, obj.postData, {});
                var result = $.grep(pd.group_data, function(e) {
                    return e.groupColumn === groupColumn;
                });
                if (result.length === 0) {
                    var o = {
                    ...
                    result[0].parentGroupValue = parentGroupValue;
                }
                pd.groupOn = groupColumn;
                if (obj.summaryData) {
                    pd.summaryData = obj.summaryData;
                }
                tmpOptions.postData = pd;
                ...
                $('#' + box_id).css({
                    ...
                    duration: 10,
                    complete: function() {
                        mainGrid.initGroup(tmpOptions, '#' + box_id, column, value);
                        //$('#' + box_id).w2groupGrid(tmpOptions);
                        //w2ui[opts.name+'_grp_'+groupColumn+event.recid].groupColumn = _.difference(opts.groupData,groupColumn)[0];
                    }
                });

            } else { // don't have a column to group on - we will render a normal grid as last level

                var pd = $.extend({}, obj.postData, {});
                var result = $.grep(pd.group_data, function(e) {
                    return e.parentGroup === parentGroup;
                });
                if (result.length === 0) {
                    var o = {
                        groupColumn: '',
                ...
                //pd.group_data = [];
                tmpOptions.postData = pd;
                //delete tmpOptions.getGroupRecordHTML; // = getRegularRecordHTML;
                //                var col = w2ui[event.target].getColumn(tmpOptions.columns[0].field);
                //                var sz = col.sizeCalculated.replace('px','');
                //                tmpOptions.columns[0].size = (sz-24)+'px';
                tmpOptions.show.expandColumn = false;
                tmpOptions.isLastGroup = true;
                $('#' + box_id).css({
                    margin: '0px',
                    padding: '0px',
                    width: '100%',
                    ...
                    duration: 10,
                    complete: function() {
                        var grid = mainGrid.initGroup(tmpOptions, '#' + box_id, column, value);
                        //$('#' + box_id).w2groupGrid(tmpOptions);
                        $.each(grid.columns, function(i, col) {
                            var firstSize = mainGrid.columns[i].sizeCalculated.replace('px', '');
                            var groupCount = i === 0 ? obj.groupData.length - 1 : 0;
                            col.size = parseInt(firstSize - 25 * groupCount) + 'px';
                        });

                        grid.resizeRecords();
                        //$('.w2ui-expanded1').remove();
                        //$('#' + box_id).prepend('<td class="w2ui-grid-data w2ui-expanded1" colspan="1"><div style="display: none"></div></td>');
                    }
                });
            }
            //w2ui[tmpOptions.name].resize();
        },
        ...
        groupCollapse: function(recid) {
            ...
            setTimeout(function() {
                ...
                setTimeout(function() {
                    ...
                    var grids = mainGrid.childrenGrids;
                    ...
                    // }, true);
                    for (var x in grids) {
                        if (x.indexOf(obj.name) !== -1 && x !== obj.name)
                            delete mainGrid.childrenGrids[x];
                    }
        ...
        getRoot: function(name) {
            return name.substring(0, name.indexOf('_grp_') + 6);
        },

        initGrouping: function() {
            //var options = this;
            var mainGrid = w2ui[this.parentGrid] ? w2ui[this.parentGrid] : this;
            var ele = $(mainGrid.box);
            $('.dropGroup .w2ui-col-header').off('click').on('click', function() {
                var el = $(this);
                el.remove();

                // var mainGrid = w2ui[w2ui[options.name].parentGrid];
                // var ele = $(mainGrid.box);
                //var id; //get the id of group, if is first, we have to rebuild grid
                $.map(mainGrid.defaultOptions.groupData, function(obj, index) {
                    if (obj.field === el.data('col'))
                        id = index;
                });


                mainGrid.groupData = $.grep(mainGrid.groupData, function(obj, index) {
                    return obj.field === el.data('col');
                }, true);

                // if (id === 0 && mainGrid.groupData.length > 0) {
                //     mainGrid.groupColumn = mainGrid.groupData[0].field;
                //     mainGrid.postData.groupOn = mainGrid.groupColumn;
                //     //     defaultOptions.postData.group_data = [{
                //     //         groupColumn: defaultOptions.groupColumn,
                //     //         parentGroup: '',
                //     //         parentGroupValue: ''
                //     //     }];
                //     //     //defaultOptions.name = mainGrid.name;
                //     //mainGrid.destroy();
                //     //ele.w2groupGrid(mainGrid);
                //     //return;
                // }

                // mainGrid.postData.group_data = $.grep(mainGrid.postData.group_data, function(obj, index) {
                //     return obj.parentGroup === el.data('col');
                // }, true);

                // $.each(mainGrid.postData.group_data, function(i, grp) {
                //     if (grp.groupColumn === el.data('col')) {
                //         grp.groupColumn = '';
                //     }
                // });
                ...
                mainGrid.groupColumn = '';
                //mainGrid.groupData = defaultOptions.groupData;
                //mainGrid.postData.group_data = defaultOptions.postData.group_data;

                if (mainGrid.groupData.length === 0) {
                    //defaultOptions.postData.groupOn = '';
                    mainGrid.postData.groupOn = '';
                    mainGrid.groupColumn = '';
                    //defaultOptions.postData.group_data = [];
                    //mainGrid.postData.group_data = [];
                    //defaultOptions.isLastGroup = true;
            ...
            function handleDragDrop(e) {
                ...
                mainGrid.groupData.push({
                    field: $(dragSrcEl).data('col')
                });
                // if (mainGrid.groupData.length === 1) {
                //     delete mainGrid.groupColumn;
                //     mainGrid.destroy();
                //     ele.w2groupedGrid(mainGrid);
                //     return;
                // }
                // mainGrid.reload();
                mainGrid.destroy();
        ...
        getGroupedRecords: function() {
            var grid = w2ui[this.parentGrid] || this;
            var obj = this;
            var parentRecords = [];
            var fields = [];
            ...
            var rs = _.groupByMulti(grid.defaultRecords, fields);
            var index = 0;
            parentRecords = obj.constructRecords(rs, this.groupColumn);
            return parentRecords;
        },

        constructRecords: function(data, groupColumn) {
            var obj = this;
            var ret = [];
            var index = 0;
            $.each(data, function(i, grp) {
                var o = {
                    recid: index,
                    grpcnt: obj.getSizeOfGroup(groupColumn, i),
                    childRecords: grp
                };
                o[groupColumn] = i;
                ret.push(o);
        ...
        getSizeOfGroup: function(column, value) {
            var count = _.countBy(this.defaultRecords, function(num) {
        ...
        getRecordsOfGroup: function(column, value) {
            var obj = this;
            var records = _.where(this.records, function(rec) {
                return rec[column] === value;
            });
        ...
        search: function(field, value) {
            var obj = w2ui[this.parentGrid] || this;
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            var searchData = [];
                ...
                if (typeof value != 'undefined') {
                    if (field.toLowerCase() == 'all') {
                        ...
                        if (this.searches.length > 0) {
                            ...
                            // to modify by getting all columns
                            for (var c in obj.defaultColumns) {
                                var tmp = {
                                    field: obj.columns[c].field,
                                    type: 'text',
                                    operator: 'contains',
                                    value: value
                                };
        ...
        "delete": function(force) {
            var obj = this;
            var grid = obj.childrenGrids[this.last.activeGrid] || this;
            console.log(grid);
            // event before
            var eventData = grid.trigger({
                phase: 'before',
            ...
            if (this.msgDelete != '' && !force) {
                w2confirm({
                    ...
                    callBack: function(result) {
                        if (result == 'Yes') grid['delete'](true);
                    }
                });
                return;
            }
            ...
            if (url) {
                grid.request('delete-records');
            } else {
                grid.selectNone();
                if (typeof recs[0] != 'object') {
                    grid.remove.apply(this, recs);
                } else {
                    // clear cells
                    for (var r in recs) {
                        var fld = grid.columns[recs[r].column].field;
                        var ind = grid.get(recs[r].recid, true);
                        if (ind != null && fld != 'recid') {
                            grid.records[ind][fld] = '';
                            if (grid.records[ind].changes) delete grid.records[ind].changes[fld];
                        }
                    }
                    grid.refresh();
                }
        ...
        click: function(recid, event) {
            ...
            if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                ...
                if (grid === this.parentGrid)
                    w2ui[grid].selectNone();
                else
                    w2ui[obj.parentGrid].childrenGrids[grid].selectNone()
                    // all subgrids
                parent.parents('.w2ui-grid').find('.w2ui-expanded-row .w2ui-grid').each(function(index, el) {
                    var grid = $(el).attr('name');
                    if (grid === obj.parentGrid) {
                        if (w2ui[grid])
                            w2ui[grid].selectNone();
                    } else {
                        if (w2ui[obj.parentGrid].childrenGrids[grid])
                            w2ui[obj.parentGrid].childrenGrids[grid].selectNone();
                    }
                });
            }
        ...
        keydown: function(event) {
            ...
            var grid = this.name === this.parentGrid ? 'w2ui[\'' + this.name + '\']' : 'w2ui[\'' + this.parentGrid + '\'].childrenGrids[\'' + this.name + '\']';
            if (eventData.isCancelled === true) return;
            // default behavior
            var empty = false;
        ...
        sort: function(field, direction, multiField) { // if no params - clears sort
            ...
            if (!url) {
                ...
                this.last.xhr_offset = 0;
                $(this.box).find('.w2ui-head [col="' + sortIndex + '"]').find('.w2ui-col-header').addClass('w2ui-col-sorted').next().addClass('w2ui-sort-up');
                if (this.isLastGroup || field === this.groupColumn) {
                    this.reload();
                } else {
                    $.each(mainGrid.childrenGrids, function(i, grid) {
                        //var grid = w2ui[gridname.grid];
                        if (grid && (grid.isLastGroup || field === grid.groupColumn)) {
                            grid.sortData = obj.sortData;
                            grid.reload();
                        }
                    });
                }
        ...
        refreshRow: function(recid) {
            ...
            if (tr.length != 0) {
                ...
                $(tr).replaceWith(this.getGroupRecordHTML(ind, line, isSummary));
        ...
        refresh: function() {
            ...
            for (var g in this.groupData) {
                ...
                $.map(this.columns, function(ob, index) {
                    if (ob.field === group.field) {
                        col = ob.caption;
                    }
                });
                $(this.box).find('.dropGroup').append('<div class="w2ui-col-header" data-col="' + group.field + '">' + col + '</div>');
            }
        ...
        destroy: function() {
            var mainGrid = w2ui[this.parentGrid] || this;
            var obj = this;
            // event before
            var eventData = this.trigger({
                phase: 'before',
            ...
            // mainGrid.expandedGrids = $.grep(mainGrid.expandedGrids, function(index, ob) {
            //     //return (name === ob.grid || name.substring(0, name.indexOf('_grp_') + 6) === ob.parent);
            //     if (ob.parent === obj.name) {
            //         if (w2ui[ob.grid])
            //             w2ui[ob.grid].destroy();
            //         return true;
            //     }
            // }, true);
        ...
        initToolbar: function() {
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                this.toolbar.on('click', function(event) {
                    ...
                    if (eventData.isCancelled === true) return;
                    var id = event.target;
                    var grid = obj.childrenGrids[obj.last.activeGrid] || obj;
                    switch (id) {
                        case 'w2ui-reload':
                            var eventData2 = obj.trigger({
                                phase: 'before',
            ...
            $(this.box).find('.w2ui-resizer')
                ...
                .on('mousedown', function(event) {
                    ...
                    var mouseMove = function(event) {
                        ...
                        var groupCount = column == 0 ? obj.groupData.length - 1 : 0;;
                        var colSize = obj.columns[obj.last.tmp.col].size.replace('px', '');
                        for (var f in mainGrid.childrenGrids) {
                            var childGrid = mainGrid.childrenGrids[f];
                            if (childGrid.isLastGroup) {
                                ...
                                childGrid.columns[obj.last.tmp.col].size = parseInt(colSize - 25 * groupCount) + 'px';
                                childGrid.resize();
        ...
        getSummaryHTML: function() {
            ...
            var html = '<table>';
            for (var i = 0; i < this.summary.length; i++) {
                html += this.getGroupRecordHTML(i, i + 1, true);
            }
            html += '</table>';
            return html;
        },
        ...
        scroll: function(event) {
            ...
            if (first < start || first == 1 || this.last.pull_refresh) { // scroll down
                ...
                for (var i = parseInt(rec_start) + 1; i <= end; i++) {
                    ...
                    tr2.before(this.getGroupRecordHTML(i - 1, i));
                }
                markSearch();
                setTimeout(function() {
                    obj.refreshRanges();
                ...
                for (var i = parseInt(rec_start) - 1; i >= start; i--) {
                    if (!this.records[i - 1]) continue;
                    if (this.records[i - 1].expanded === true) this.records[i - 1].expanded = false;
                    tr1.after(this.getGroupRecordHTML(i - 1, i));
                }
                markSearch();
                setTimeout(function() {
                    obj.refreshRanges();
        ...
        getGroupRecordHTML: function(ind, lineNum, summary) {
            var rec_html = '';
            var grid = this.name === this.parentGrid ? 'w2ui[\'' + this.name + '\']' : 'w2ui[\'' + this.parentGrid + '\'].childrenGrids[\'' + this.name + '\']';
            var sel = this.last.selection;
            var record;
            ...
            rec_html += '<tr id="grid_' + this.name + '_rec_' + record.recid + '" recid="' + record.recid + '" line="' + lineNum + '" ' +
                ...
                (summary !== true ?
                    (w2utils.isIOS ?
                        '    onclick  = "' + grid + '.dblClick(\'' + record.recid + '\', event);"' :
                        '    onclick  = "' + grid + '.click(\'' + record.recid + '\', event);"' +
                        '    oncontextmenu = "' + grid + '.contextMenu(\'' + record.recid + '\', event);"'
                    ) : ''
                ) +
            ...
            if (this.show.expandColumn) {
                ...
                rec_html +=
                    ...
                    (summary !== true ?
                        ...
                        '            onclick="' + grid + '.toggleGroup(\'' + record.recid + '\', event); ' +
                        '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">' +
                        '        ' + tmp_img + ' </div>' :
                        '') +
                    '</td>';
            ...
            if (this.isLastGroup) {
                while (true) {
                    ...
                    var rec_cell = this.getCellHTML(ind, col_ind, summary, this.isLastGroup);
                    var addStyle = '';
                    ...
                    rec_html += '<td ' + (!this.isLastGroup ? 'colspan="' + colspan + '"' : '') + ' class="w2ui-grid-data' + (isCellSelected ? ' w2ui-selected' : '') + (isChanged ? ' w2ui-changed' : '') + '" col="' + col_ind + '" ' +
            ...
            var col, groupCol = {};
            for (var y in this.groupTemplates) {
                if (summary !== true) {
                    if (this.groupTemplates[y].field === this.groupColumn) {
                        groupCol = this.groupTemplates[y];
                        break;
                    }
                } else {
                    if (this.summaryTemplates[y] && this.summaryTemplates[y].field === this.groupColumn) {
                        groupCol = this.summaryTemplates[y];
                        break;
                    }
                }
            ...
            for (var x in this.columns) {
                if (this.columns[x].field === this.groupColumn) {
                    col = this.columns[x];
                    // if (groupCol.render) {
                    //     col.origRender = $.extend(col.render, true);
                    //     col.render = $.extend(groupCol.render, true);
                    // }
                    col_ind = x;
                    break;
                }
            ...
            var colspan = this.columns.length;
            // = this.columns.indexOf(this.groupColumn);
            //var col = this.columns[col_ind];
            var rec_cell = this.getCellHTML(ind, col_ind, summary, this.isLastGroup);
            var addStyle = '';
            if (typeof col.render == 'string') {
            ...
            var isCellSelected = false;
            if (isRowSelected && $.inArray(col_ind, sel.columns[ind]) != -1) isCellSelected = true;
            rec_html += '<td colspan="' + colspan + '" class="w2ui-grid-data' + (isCellSelected ? ' w2ui-selected' : '') + '" col="' + col_ind + '" ' +
                '    style="' + addStyle + (typeof col.style != 'undefined' ? col.style : '') + '" ' +
                (typeof col.attr != 'undefined' ? col.attr : '') + '>' +
                rec_cell +
                '</td>';
        ...
        getCellHTML: function(ind, col_ind, summary, isLastGroup) {
            var col = this.columns[col_ind];
            if (!isLastGroup) {
                for (var y in this.groupTemplates) {
                    if (summary !== true) {
                        if (this.groupTemplates[y].field === this.groupColumn) {
                            col = this.groupTemplates[y];
                            break;
                        }
                    } else {
                        if (this.summaryTemplates[y] && this.summaryTemplates[y].field === this.groupColumn) {
                            col = this.summaryTemplates[y];
                            break;
                        }
                    }
                }
    ...
    $.extend(w2groupGrid.prototype, w2utils.event);
    w2obj.groupGrid = w2groupGrid;
})();
