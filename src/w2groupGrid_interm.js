    ...
    w2groupGrid.prototype = {
        ...
        initGroup: function(method, box) {
            if (typeof method === 'object' || !method) {
                ...
                // register new object
                w2ui[object.parentGrid].childrenGrids.push(object);
                return object;
        ...
        showGroup: function(recid) {
            ...
            tmpOptions.sortData = w2ui[obj.name].sortData;

            tmpOptions.searchData = w2ui[obj.name].searchData;
            ...
            var record = w2ui[obj.name].get(recid); //get the expanded record
            ...
            if (groupColumn) { // we have a column to group on - we will render a grouped grid
                ...
                $('#' + box_id).css({
                    ...
                    duration: 10,
                    complete: function() {
                        mainGrid.initGroup(tmpOptions, '#' + box_id);
                ...
                $('#' + box_id).css({
                    ...
                    complete: function() {
                        mainGrid.initGroup(tmpOptions, '#' + box_id);
                        //$('#' + box_id).w2groupGrid(tmpOptions);
                        $.each(w2ui[tmpOptions.name].columns, function(i, col) {
                            var firstSize = w2ui[parent].columns[i].sizeCalculated.replace('px', '');
                            var groupCount = i === 0 ? obj.groupData.length - 1 : 0;
                            col.size = parseInt(firstSize - 25 * groupCount) + 'px';
                        });

                        w2ui[tmpOptions.name].resizeRecords();
        ...
        groupCollapse: function(recid) {
            ...
            setTimeout(function() {
                ...
                setTimeout(function() {
                    ...
                    //mainGrid.expandedGrids = $.grep(mainGrid.expandedGrids, function(ob) {
        ...
        getGroupedRecords: function() {
            ...
            var obj = this;
            if (this.isLastGroup) {
                return obj.getRecordsOfGroup(grid.defaultRecords);
            }
            var recs = obj.getRecordsOfGroup(grid.defaultRecords);
            var rs = [];
            var groups = _.groupBy(recs, this.groupColumn);
            var index = 0;
        ...
        getRecordsOfGroup: function(records) {
            var grid = w2ui[this.parentGrid] || this;
            var obj = this;
            var filter = {};
            var x;
            $.each(grid.postData.group_data, function(i, data) {
                if (data.groupColumn === obj.postData.groupOn) {
                    x = i;
                }
            });
            //var applyFilter {};
            for (var y = x; y >= 0; y--) {
                var f = grid.postData.group_data[y];
                filter[f.parentGroup] = f.parentGroupValue;
            }
            return _.where(records, filter);
        },
        ...
        "delete": function(force) {
            var obj = this;
            var grid = w2ui[this.last.activeGrid] || this;
            console.log(grid);
            // event before
            var eventData = grid.trigger({
                phase: 'before',
        ...
        sort: function(field, direction, multiField) { // if no params - clears sort
            ...
            if (!url) {
                ...
                if (this.isLastGroup || field === this.groupColumn) {
                    ...
                    $.each(mainGrid.expandedGrids, function(i, gridname) {
        ...
        initToolbar: function() {
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                this.toolbar.on('click', function(event) {
                    ...
                    if (eventData.isCancelled === true) return;
                    var id = event.target;
                    var grid = w2ui[obj.last.activeGrid] || obj;
                    switch (id) {
                        case 'w2ui-reload':
                            var eventData2 = obj.trigger({
                                phase: 'before',
            ...
            $(this.box).find('.w2ui-resizer')
                ...
                .on('mousedown', function(event) {
                    ...
                    var mouseMove = function(event) {
                        ...
                        for (var f in mainGrid.expandedGrids) {
                            var childGrid = w2ui[mainGrid.expandedGrids[f].grid];
