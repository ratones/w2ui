    ...
    $.fn.w2groupGrid = function(method) {
        if (typeof method === 'object' || !method) {
            ...
            if (method.groupData && method.groupData.length > 0) {
                if (!method.groupColumn) { // we are at first level, so we take first column as groupColumn
                    method.groupColumn = method.groupData[0].field;
            ...
            // $.each(method.groupData, function(i, col) {
            //     object.hideColumn(col.field);
            // });

            object.show.expandColumn = object.groupData.length > 0;
            //$.extend(true, object.toolbar, toolbar);

            // if (method.isLastGroup) {
            //     for (var x in columns) {
            //         var c = columns[x];
            //         if (c.origRender)
            //             c.render = c.origRender;
            //     }
            // }
            // reassign variables
             ...
             * @type {[type]}
             */
            method.postData = method.postData || {};
            method.postData.group_data = method.postData.group_data || [];
            ...
            //extend postData with grouping and summary data
            //var newPostData = $.extend({}, options.postData, pd);
            //options.postData = pd;

            object.postData = $.extend(postData, pd);
            object.isLastGroup = object.groupData.length === 0;
    ...
    w2groupGrid.prototype = {
        ...
        reinit: function() {
            ...
            if (this.groupData && this.groupData.length > 0) {
                if (!this.groupColumn) { // we are at first level, so we take first column as groupColumn
                    this.groupColumn = this.groupData[0].field;
                    this.parentGrid = this.name; // this is parent grid
                }
            }
            var object = new w2groupGrid(this);
            $.extend(object, {
                postData: {},
                records: [],
                columns: [],
            ...
            if (object.groupData.length > 0) object.show.expandColumn = true;
            ...
            this.postData.group_data = this.postData.group_data || [];

            // setting additional postData
            var pd = $.extend(this.postData, true);
            var result = $.grep(pd.group_data, function(e) {
                return e.parentGroup === this.parentGroup;
            });

            if (result.length === 0) { // if item not exists, we create one taking data that comes from expanded grid
                var o = {
                    groupColumn: this.groupColumn,
                    parentGroup: this.parentGroup,
                    parentGroupValue: this.parentGroupValue
                };
                pd.group_data.push(o);
            } else { //we have to modify the group_data if an item already exists
                result[0].parentGroupValue = this.parentGroupValue;
                result[0].parentGroup = this.parentGroup;
                result[0].groupColumn = this.groupColumn;
            }
            pd.groupOn = this.groupColumn;
            if (this.summaryData) {
                pd.summaryData = this.summaryData;
            }
            //extend postData with grouping and summary data
            //var newPostData = $.extend({}, options.postData, pd);
            //options.postData = pd;
            ...
            // render if necessary
            //if ($(this).length !== 0) {
            object.render($(w2ui[this.parentGrid].box));
            //}
        ...
        showGroup: function(recid) {
            ...
            var isexpanded = $.grep(expandedGrids, function(e) {
                return e.grid === name;
            });
            ...
            var tmpOptions = $.extend({}, {}, defaultOptions); //extend options
            ...
            if (groupColumn) { // we have a column to group on - we will render a grouped grid
                ...
                // if (obj.groupData.length === 1) {
                //     tmpOptions.onExpand = null;
                // }
                ...
                $('#' + box_id).css({
                    ...
                    duration: 10,
                    complete: function() {
                        $('#' + box_id).w2grid(tmpOptions);
                        $.each(w2ui[tmpOptions.name].columns, function(i, col) {
                            var firstSize = w2ui[obj.parentGrid].columns[i].sizeCalculated.replace('px', '');
        ...
        groupCollapse: function(recid) {
            ...
            setTimeout(function() {
                ...
                setTimeout(function() {
                    ...
                    expandedGrids = $.grep(expandedGrids, function(ob) {
        ...
        initGrouping: function() {
            ...
            $('.dropGroup .w2ui-col-header').off('click').on('click', function() {
                ...
                $.map(defaultOptions.groupData, function(obj, index) {
        ...
        sort: function(field, direction, multiField) { // if no params - clears sort
            ...
            if (!url) {
                ...
                if (this.isLastGroup || field === this.groupColumn) {
                    ...
                    $.each(expandedGrids, function(i, gridname) {
            ...
            $(this.box).find('.w2ui-resizer')
                ...
                .on('mousedown', function(event) {
                    ...
                    var mouseMove = function(event) {
                        ...
                        for (var f in expandedGrids) {
                            var childGrid = w2ui[expandedGrids[f].grid];
