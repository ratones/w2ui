...
var w2utils = (function() {
    ...
    function validate(model) {
        ...
        for (var f in model.fields) {
            ...
            if (field.required && (val === '' || !val || ($.isArray(val) && val.length === 0) || ($.isPlainObject(val) && $.isEmptyObject(val)))) {
                errors.push({
                    field: field,
...
(function($) {
    ...
    */
    ...
    w2field.prototype = {
        ...
        cascadeTo: function(options) {
            var obj = this;
            // var selected = $(obj.el).data('selected');
            for (var i in options) {
                // var value;
                // if (selected)
                //     value = selected[options[i].field];
                var el = $(options[i].el).w2field();
                if (el /*&& value*/ ) {
                    el.options.items = [];
                    // $(options[i].el).data('selected', null);
                    // var pd = {};
                    // pd[options[i].postDataName] = value;
                    // $.extend(el.options.postData, pd);
                    el.clearCache();
                    el.tmp.xhr_search = 'undefined';
                    el.tmp.xhr_total = 'undefined';
                    el.tmp.search = '';
                    // console.log('reset field' + options[i].el);
                    //el.reset();
                    //el.request();
                }
            }
